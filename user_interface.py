...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                             ...
                             QComboBox, QGroupBox,
                             QProgressBar, QMessageBox, QAction, QMenuBar, QSizePolicy)
                    ...
                    if ret:
                        try:
                            ...
                            for entry in regions:
                                if len(entry) >= 3:
                                    color, roi, bbox = entry[0], entry[1], entry[2]
...
class NavigationInterface(QMainWindow):
    ...
    def _init_ui(self):
        ...
        # Allow the central widget to expand and fill the available space
    ...
    def _switch_theme(self, theme_name):
        ...
        if theme_name in THEMES:
            ...
            self._update_widget_styles(theme_name)
    ...
    def _toggle_high_contrast(self):
        ...
        self._update_widget_styles(UI_SETTINGS['theme'])
    ...
    def _apply_theme(self, theme_name):
        ...
        if UI_SETTINGS['high_contrast']:
            ...
            if theme_name == 'light':
                ...
                palette.setColor(QPalette.ButtonText, QColor(0, 0, 0)) # Ensure black text on light buttons
                ...
                palette.setColor(QPalette.ButtonText, QColor(255, 255, 255)) # Ensure white text on dark buttons
                palette.setColor(QPalette.Highlight, QColor(0, 120, 215))
                palette.setColor(QPalette.HighlightedText, QColor(255, 255, 255))
        else:
            # Normal theme mode
    ...
    def _update_widget_styles(self, theme_name):
        """Update stylesheets for all widgets to reflect the current theme."""
        if theme_name not in THEMES:
            return
        ...
        base_style = f"""
            ...
            QPushButton:disabled {{
                background-color: {theme.get('disabled_bg', '#888')};
                color: {theme.get('disabled_text', '#ccc')};
            }}
            QTextEdit, QComboBox {{
                border: 1px solid {theme['border_color']};
                background-color: {theme['window_bg']};
        ...
        if hasattr(self, 'status_indicator'):
            status_color = theme['status_offline']  # Default to offline
            self.status_indicator.setStyleSheet(f"color: {status_color}; font-size: 24px;")

        if hasattr(self, 'camera_display'):
            self.camera_display.setStyleSheet(f"border: 2px solid {theme['border_color']}; background-color: black; color: white;")

        if hasattr(self, 'qr_status_label'):
            self.qr_status_label.setStyleSheet(f"color: {theme['warning_color']};")

        if hasattr(self, 'location_label'):
            self.location_label.setStyleSheet(f"color: yellow; font-weight: bold;")
    ...
    def _create_camera_section(self, layout):
        ...
        # Allow the camera display to expand
        ...
        self.qr_status_label.setFont(self._get_font('font_size_medium'))
    ...
    def _create_navigation_section(self, layout):
        ...
        self.location_label.setFont(self._get_font('font_size_medium'))
        ...
        self.destination_combo.setFont(self._get_font('font_size_medium'))
        ...
        self.calculate_route_btn.setFont(self._get_font('font_size_medium'))
        ...
        self.route_display.setFont(self._get_font('font_size_small'))
    ...
    def _create_status_section(self, layout):
        ...
        self.status_log.setFont(self._get_font('font_size_small'))
    ...
    def _get_font(self, size_key):
        """Get a font with a specified size from UI_SETTINGS."""
        font = QFont()
        font.setPointSize(UI_SETTINGS[size_key])
        return font
    ...
    def _start_camera(self):
        ...
        try:
            ...
            theme = THEMES[UI_SETTINGS['theme']]
            self.status_indicator.setStyleSheet(f"color: {theme['status_online']}; font-size: 24px;")
    ...
    def _stop_camera(self):
        ...
        self.stop_camera_btn.setEnabled(False)
        theme = THEMES[UI_SETTINGS['theme']]
        self.status_indicator.setStyleSheet(f"color: {theme['status_offline']}; font-size: 24px;")
        ...
        try:
            ...
            if data:
                ...
                if location_data.is_valid():
                    ...
                    theme = THEMES[UI_SETTINGS['theme']]
                    self.qr_status_label.setStyleSheet(f"color: {theme['status_online']};")
